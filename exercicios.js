const exerciseDB = {
    "Peitoral": [
        // EXEMPLO 1: Com Imagem Real (Wikimedia)
        { 
            name: "Crucifixo (Cross Over) com Halteres", 
            img: "https://upload.wikimedia.org/wikipedia/commons/7/74/DumbbellFlye.gif" 
        },
        // EXEMPLO 2: Estrutura pronta para você colar um link depois
        { 
            name: "Supino Reto com Barra", 
            img: ""https://i.pinimg.com/originals/8e/34/bb/8e34bb41d30ceb2f65aa7873a87a4371.gif"
        },
        // EXEMPLO 3: Texto Simples (Funciona igual antes)
        "Supino Inclinado com Barra",
        "Supino Declinado com Barra",
        "Supino Reto com Halteres",
        "Supino Inclinado com Halteres",
        "Supino Declinado com Halteres",
        "Flexão de Braço (Barra Fixa)",
        "Mergulho entre Bancos (Paralelas)",
        "Pullover com Haltere",
        "Supino Máquina (Guizado)",
        "Voador (Peck-Deck ou Butterfly)",
        "Cross Over (Polia)",
        "Máquina de Supino Inclinado"
    ],
    "Costas": [
        "Barra Fixa (Pronada/Supinada/Neutra)",
        "Puxada Alta com Barra",
        "Remada Curvada com Barra",
        "Remada Unilateral com Haltere",
        "Remada Serrote (com haltere)",
        "Pullover com Haltere",
        "Pulley Frente (Puxada Alta)",
        "Pulley Atrás",
        "Remada Máquina (Sentado)",
        "Remada Cavalinho (Apoio Peito)",
        "Máquina de Barra Fixa Assistida",
        "Barra Fixa Pegada Neutra",
        "Barra Fixa Pegada Pronada",
        "Barra Fixa Negativa",
        "Barra Fixa c/ Elástico"
    ],
    "Ombros": [
        "Desenvolvimento Militar com Barra",
        "Desenvolvimento com Halteres",
        "Elevação Lateral com Halteres",
        "Elevação Frontal (Halteres/Barra)",
        "Remada Alta (Barra/Halteres)",
        "Crucifixo Invertido (Halteres)",
        "Encolhimento (Shrug)",
        "Desenvolvimento Máquina",
        "Elevação Lateral Máquina",
        "Crucifixo Invertido no Peck-Deck",
        "Máquina de Encolhimento"
    ],
    "Pernas": [
        "Agachamento Livre (Back Squat)",
        "Agachamento Frontal (Front Squat)",
        "Agachamento Sumô",
        "Afundo (Lunge)",
        "Passada (Walking Lunge)",
        "Agachamento Búlgaro",
        "Agachamento Hack (Solo)",
        "Cadeira Extensora",
        "Leg Press 45°",
        "Máquina de Agachamento Hack",
        "Máquina Smith (Agachamento)",
        "Stiff (Levantamento Terra Romeno)",
        "Mesa Flexora",
        "Elevação Pélvica (Hip Thrust)",
        "Cadeira Flexora",
        "Panturrilha em Pé (Livre)",
        "Panturrilha Sentado (Livre)",
        "Máquina de Panturrilha em Pé",
        "Máquina de Panturrilha Sentado",
        "Leg Press (Panturrilha)"
    ],
    "Braços": [
        "Rosca Direta (Barra Reta/W)",
        "Rosca Alternada com Halteres",
        "Rosca Scott (Livre)",
        "Rosca Concentrada",
        "Rosca Martelo",
        "Rosca Scott Máquina",
        "Rosca no Banco Preacher",
        "Mergulho entre Bancos (Tríceps)",
        "Supino Fechado",
        "Tríceps Testa (Skull Crusher)",
        "Tríceps Corda/Barra (Pulley)",
        "Tríceps Coice (Kickback)",
        "Máquina de Mergulho Assistido",
        "Extensão de Tríceps na Máquina"
    ],
    "Abdômen e Core": [
        "Prancha Abdominal (Front Plank)",
        "Prancha Lateral (Side Plank)",
        "Abdominal Crunch (Solo)",
        "Abdominal Reverso",
        "Abdominal Bicicleta",
        "Russian Twist",
        "Mountain Climber (Escalador)",
        "Elevação de Pernas Suspenso",
        "Abdominal na Máquina",
        "Abdominal Banco Declinado",
        "Roda Abdominal"
    ]
};